...
let lastComplaintTimestamp = null;
const complaints = [
...
function updateLiveWaterQuality() {
    fetch('http://localhost:3000/liveWaterQuality')
        ...
        .then(data => {
            ...
            if (data.tds > 500 || data.turbidity > 25) {
                document.getElementById('status-label').textContent = "❌ Bad Quality";
                document.getElementById('status-label').style.color = "red";
            } else {
                document.getElementById('status-label').textContent = "✅ Good Quality";
...
setInterval(updateLiveWaterQuality, 3000);
...
function showWaterQuality() {
    ...
    fetch('http://localhost:3000/liveWaterQuality')
        ...
        .then(data => {
            ...
            if (data.tds > 500 || data.turbidity > 25) {
                statusEl.textContent = "❌ Bad Quality";
                statusEl.style.color = "red";

                // 🚫 Complaint is no longer auto-lodged here
                console.log("⚠️ Bad water quality detected, but complaint not auto-lodged from UI.");
            } else {
